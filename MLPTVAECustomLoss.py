...
feet_length_criterion = nn.MSELoss()
...
scaling_vars, scaling_means = training_prep.get_scale_back_feet_mats()
scaling_vars = to_torch(scaling_vars)
scaling_means = to_torch(scaling_means)
...
def get_feet_length(feet_pos):
    ...
    # hip - LeftLeg
    h_ll = original_feet_pos[:, :3] - original_feet_pos[:, 9:12]
    # hip - RightLeg
    h_rl = original_feet_pos[:, :3] - original_feet_pos[:, 12:15]

    # LeftLeg - LeftFoot
    ll_lf = original_feet_pos[:, 9:12] - original_feet_pos[:, 3:6]
    # RightLeg - RightFoot
    rl_rf = original_feet_pos[:, 12:15] - original_feet_pos[:, 6:9]

    get_length = lambda x : torch.sum(x * x, 1).unsqueeze(0)
    ...
    return to_torch(torch.vstack((h_ll,h_rl,ll_lf,rl_rf)))

default_feet_length = get_feet_length(train_feet_output[0:1,:])
# default_feet_length = default_feet_length.unsqueeze()
...
for epoch in range(num_epochs):
    for i in range(int(np.floor(train_feet_input.shape[0] / batch_size))):
        ...
        feet_lengths = get_feet_length(model_output)
        target_lengths = default_feet_length.repeat(model_output.shape[0], 1)
        model_feet_length = get_feet_length(model_output)
        loss = feet_criterion(model_output, target_output) + feet_length_criterion(model_feet_length, default_feet_length)
...
vels_and_accels = torch.hstack((eval_input[STACKCOUNT - 1, 0:6], eval_input[STACKCOUNT - 1, -18:]))
...
with torch.no_grad():
    for curr_eval_idx in range(STACKCOUNT, eval_input.shape[0]):
        ...
        vels_and_accels = torch.hstack((eval_input[curr_eval_idx, 0:6], eval_input[curr_eval_idx, -18:]))
        curr_input_mat[-1] = model_output
        curr_input_mat[-1][:3] = target_output[curr_eval_idx, 6:9]

        new_input_feet = torch.flatten(curr_input_mat)

target_output[STACKCOUNT:, 6:15] = lower_body_poses[:, :9]
target_output[STACKCOUNT:, 21:27] = lower_body_poses[:, 9:]
